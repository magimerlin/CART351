 ...
 if($_FILES)
  {
   ...
   move_uploaded_file($_FILES['filename']['tmp_name'], "images/".$fname);
    ...
    $myPackagedData=new stdClass(); //most generic class
    $myPackagedData->artist = $artist ; //arrow is like dot notation
    ...
    $myJSONObj = json_encode($myPackagedData); //takes PHP converts to JSON string
    echo $myJSONObj;
    exit;//goodbye PHP we've had enough
...
$(document).ready (function(){
    $("#insertGallery").submit(function(event) {
     ...
     $.ajax({
            ...
            contentType: false,
            cache: false, //ensures server will take new one not create a historical one
            timeout: 600000,
